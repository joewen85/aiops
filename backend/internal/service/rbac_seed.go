...
func permissionSeeds() []models.Permission {
	...
	addButton("button.tickets.ticket.comment", "评论工单")
	addButton("button.tickets.ticket.link", "关联工单对象")
	addButton("button.tickets.ticket.operation", "执行工单动作")
	...
	addButton("button.docker.host.check", "校验Docker主机")
	...
	addButton("button.middleware.metrics.collect", "采集中间件指标")
	addButton("button.middleware.action.run", "执行中间件动作")
	...
	addButton("button.kubernetes.cluster.check", "校验K8s集群")
	addButton("button.kubernetes.cluster.sync", "同步K8s资源")
	addButton("button.kubernetes.resource.view", "查看K8s资源")
	addButton("button.kubernetes.resource.create", "创建K8s资源")
	addButton("button.kubernetes.resource.update", "更新K8s资源")
	addButton("button.kubernetes.resource.delete", "删除K8s资源")
	addButton("button.kubernetes.resource.action", "执行K8s资源动作")
	addButton("button.kubernetes.node.register", "注册K8s节点")
	addButton("button.kubernetes.node.register_task", "任务编排注册K8s节点")
	addButton("button.kubernetes.operation.view", "查看K8s操作记录")
	...
	addAPI("api.tickets.ticket.submit", "POST", "/api/v1/tickets/:id/submit", "提交工单")
	addAPI("api.tickets.ticket.cancel", "POST", "/api/v1/tickets/:id/cancel", "取消工单")
	addAPI("api.tickets.ticket.reopen", "POST", "/api/v1/tickets/:id/reopen", "重开工单")
	addAPI("api.tickets.ticket.approve", "POST", "/api/v1/tickets/:id/approve", "审批工单")
	addAPI("api.tickets.ticket.reject", "POST", "/api/v1/tickets/:id/reject", "驳回工单")
	addAPI("api.tickets.ticket.transfer", "POST", "/api/v1/tickets/:id/transfer", "转派工单")
	addAPI("api.tickets.ticket.add_approver", "POST", "/api/v1/tickets/:id/add-approver", "添加工单审批人")
	addAPI("api.tickets.ticket.transition", "POST", "/api/v1/tickets/:id/transition", "流转工单")
	addAPI("api.tickets.ticket.flows", "GET", "/api/v1/tickets/:id/flows", "查询工单流转")
	addAPI("api.tickets.ticket.timeline", "GET", "/api/v1/tickets/:id/timeline", "查询工单时间线")
	addAPI("api.tickets.ticket.approvals", "GET", "/api/v1/tickets/:id/approvals", "查询工单审批")
	addAPI("api.tickets.ticket.comments", "GET", "/api/v1/tickets/:id/comments", "查询工单评论")
	addAPI("api.tickets.ticket.comment_create", "POST", "/api/v1/tickets/:id/comments", "创建工单评论")
	addAPI("api.tickets.ticket.attachments", "GET", "/api/v1/tickets/:id/attachments", "查询工单附件")
	addAPI("api.tickets.ticket.attachment_create", "POST", "/api/v1/tickets/:id/attachments", "创建工单附件")
	addAPI("api.tickets.ticket.links", "GET", "/api/v1/tickets/:id/links", "查询工单关联对象")
	addAPI("api.tickets.ticket.link_create", "POST", "/api/v1/tickets/:id/links", "创建工单关联对象")
	addAPI("api.tickets.ticket.link_delete", "DELETE", "/api/v1/tickets/:id/links/:linkId", "删除工单关联对象")
	addAPI("api.tickets.ticket.operations", "GET", "/api/v1/tickets/:id/operations", "查询工单操作记录")
	addAPI("api.tickets.ticket.operation_dry_run", "POST", "/api/v1/tickets/:id/operations/dry-run", "工单操作DryRun")
	addAPI("api.tickets.ticket.operation_execute", "POST", "/api/v1/tickets/:id/operations/execute", "执行工单操作")
	addAPI("api.tickets.ticket.operation_retry", "POST", "/api/v1/tickets/:id/operations/:operationId/retry", "重试工单失败操作")
	addAPI("api.tickets.sla.job.create", "POST", "/api/v1/tickets/sla/jobs", "创建工单SLA检查任务")
	addAPI("api.tickets.sla.job.get", "GET", "/api/v1/tickets/sla/jobs/:id", "查询工单SLA检查任务")
	addAPI("api.tickets.template.list", "GET", "/api/v1/ticket-templates", "查询工单模板")
	addAPI("api.tickets.template.create", "POST", "/api/v1/ticket-templates", "创建工单模板")
	addAPI("api.tickets.template.update", "PUT", "/api/v1/ticket-templates/:id", "更新工单模板")
	addAPI("api.tickets.template.delete", "DELETE", "/api/v1/ticket-templates/:id", "删除工单模板")
	addAPI("api.tickets.aiops.protocol", "GET", "/api/v1/tickets/aiops/protocol", "查询工单AIOps协议")
	addAPI("api.tickets.aiops.context", "GET", "/api/v1/tickets/aiops/context", "读取工单AIOps上下文")
	addAPI("api.tickets.aiops.intents", "POST", "/api/v1/tickets/aiops/intents", "解析工单AIOps意图")
	addAPI("api.tickets.aiops.dry_run", "POST", "/api/v1/tickets/aiops/dry-run", "工单AIOps DryRun")
	...
	addAPI("api.docker.host.check", "POST", "/api/v1/docker/hosts/:id/check", "校验Docker主机")
	addAPI("api.docker.host.resources", "GET", "/api/v1/docker/hosts/:id/resources", "查询Docker主机资源")
	...
	addAPI("api.docker.operations.list", "GET", "/api/v1/docker/operations", "查询Docker操作记录")
	addAPI("api.docker.aiops.protocol", "GET", "/api/v1/docker/aiops/protocol", "查询Docker AIOps协议")
	...
	addAPI("api.middleware.metrics.list", "GET", "/api/v1/middleware/instances/:id/metrics", "查询中间件指标")
	addAPI("api.middleware.metrics.collect", "POST", "/api/v1/middleware/instances/:id/metrics/collect", "采集中间件指标")
	addAPI("api.middleware.operations.list", "GET", "/api/v1/middleware/operations", "查询中间件操作记录")
	addAPI("api.middleware.operation.get", "GET", "/api/v1/middleware/operations/:id", "查询中间件操作详情")
	addAPI("api.middleware.aiops.protocol", "GET", "/api/v1/middleware/aiops/protocol", "查询中间件AIOps协议")
	addAPI("api.middleware.action.run", "POST", "/api/v1/middleware/actions", "执行中间件动作")
	addAPI("api.middleware.instance.action.compat", "POST", "/api/v1/middleware/instances/:id/action", "兼容执行中间件动作")
	...
	addAPI("api.kubernetes.cluster.check", "POST", "/api/v1/kubernetes/clusters/:id/check", "校验K8s集群")
	addAPI("api.kubernetes.cluster.sync", "POST", "/api/v1/kubernetes/clusters/:id/sync", "同步K8s资源")
	addAPI("api.kubernetes.resource.list", "GET", "/api/v1/kubernetes/resources", "查询K8s资源快照")
	addAPI("api.kubernetes.resource.create", "POST", "/api/v1/kubernetes/resources", "创建K8s资源")
	addAPI("api.kubernetes.resource.get", "GET", "/api/v1/kubernetes/resources/:id", "查询K8s资源详情")
	addAPI("api.kubernetes.resource.manifest", "GET", "/api/v1/kubernetes/resources/:id/manifest", "查询K8s资源Manifest")
	addAPI("api.kubernetes.resource.update", "PUT", "/api/v1/kubernetes/resources/:id", "更新K8s资源")
	addAPI("api.kubernetes.resource.delete", "DELETE", "/api/v1/kubernetes/resources/:id", "删除K8s资源")
	addAPI("api.kubernetes.node.list", "GET", "/api/v1/kubernetes/nodes", "查询K8s节点列表")
	addAPI("api.kubernetes.node.register", "POST", "/api/v1/kubernetes/nodes/register", "注册主机为K8s节点")
	addAPI("api.kubernetes.node.register.task", "POST", "/api/v1/kubernetes/nodes/register/task", "任务编排注册主机为K8s节点")
	addAPI("api.kubernetes.operations.list", "GET", "/api/v1/kubernetes/operations", "查询K8s操作记录")
	addAPI("api.kubernetes.operation.get", "GET", "/api/v1/kubernetes/operations/:id", "查询K8s操作详情")
	addAPI("api.kubernetes.aiops.protocol", "GET", "/api/v1/kubernetes/aiops/protocol", "查询K8s AIOps协议")
	addAPI("api.kubernetes.action.dry_run", "POST", "/api/v1/kubernetes/actions/dry-run", "K8s资源动作DryRun")
	addAPI("api.kubernetes.action.run", "POST", "/api/v1/kubernetes/actions", "执行K8s资源动作")
	addAPI("api.kubernetes.resource.action", "POST", "/api/v1/kubernetes/resources/action", "兼容执行K8s资源动作")
