...
func registerPhase2Routes(r *gin.RouterGroup, h *handler.Handler) {
	...
	r.POST("/ticket-templates", h.CreateTicketTemplate)
	r.PUT("/ticket-templates/:id", h.UpdateTicketTemplate)
	r.DELETE("/ticket-templates/:id", h.DeleteTicketTemplate)
	r.GET("/tickets/:id", h.GetTicket)
	r.PUT("/tickets/:id", h.UpdateTicket)
	r.DELETE("/tickets/:id", h.DeleteTicket)
	r.POST("/tickets/:id/submit", h.SubmitTicket)
	r.POST("/tickets/:id/cancel", h.CancelTicket)
	r.POST("/tickets/:id/reopen", h.ReopenTicket)
	r.POST("/tickets/:id/approve", h.ApproveTicket)
	r.POST("/tickets/:id/reject", h.RejectTicket)
	r.POST("/tickets/:id/transfer", h.TransferTicket)
	r.POST("/tickets/:id/add-approver", h.AddTicketApprover)
	r.POST("/tickets/:id/transition", h.TransitionTicket)
	r.GET("/tickets/:id/flows", h.ListTicketFlows)
	r.GET("/tickets/:id/timeline", h.TicketTimeline)
	r.GET("/tickets/:id/approvals", h.ListTicketApprovals)
	r.GET("/tickets/:id/comments", h.ListTicketComments)
	r.POST("/tickets/:id/comments", h.CreateTicketComment)
	r.GET("/tickets/:id/attachments", h.ListTicketAttachments)
	r.POST("/tickets/:id/attachments", h.CreateTicketAttachment)
	r.GET("/tickets/:id/links", h.ListTicketLinks)
	r.POST("/tickets/:id/links", h.CreateTicketLink)
	r.DELETE("/tickets/:id/links/:linkId", h.DeleteTicketLink)
	...
	r.POST("/kubernetes/clusters/:id/check", h.CheckKubernetesCluster)
	r.POST("/kubernetes/clusters/:id/sync", h.SyncKubernetesCluster)
	r.GET("/kubernetes/resources", h.ListKubernetesResources)
	r.POST("/kubernetes/resources", h.CreateKubernetesResource)
	r.GET("/kubernetes/resources/:id", h.GetKubernetesResource)
	r.GET("/kubernetes/resources/:id/manifest", h.GetKubernetesResourceManifest)
	r.PUT("/kubernetes/resources/:id", h.UpdateKubernetesResource)
	r.DELETE("/kubernetes/resources/:id", h.DeleteKubernetesResource)
	r.GET("/kubernetes/nodes", h.ListKubernetesNodes)
	r.POST("/kubernetes/nodes/register", h.RegisterKubernetesNode)
	r.POST("/kubernetes/nodes/register/task", h.RegisterKubernetesNodeTask)
	r.GET("/kubernetes/operations", h.ListKubernetesOperations)
	r.GET("/kubernetes/operations/:id", h.GetKubernetesOperation)
	r.GET("/kubernetes/aiops/protocol", h.KubernetesAIOpsProtocol)
	r.POST("/kubernetes/actions/dry-run", h.KubernetesActionDryRun)
	r.POST("/kubernetes/actions", h.KubernetesAction)
